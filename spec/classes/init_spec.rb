  ...
  describe 'with parameter require_single_user_mode_password' do
    [true,false].each do |value|
      context "set to #{value}" do
        ...
        if value.to_s == 'true'
          it { should contain_file('inittab').with_content(%r{^\s*~~:S:wait:\/sbin\/sulogin$}) }
        end

        if value.to_s == 'false'
          it { should contain_file('inittab').without_content(%r{^\s*~~:S:wait:\/sbin\/sulogin$}) }
        end
      end
    end
        ...
        platforms.sort.each do |k,v|
          ...
          context "#{v[:osfamily]} #{v[:release]}" do
            ...
            if v[:support_ctrlaltdel_override].to_s == 'true' and enable_ctrlaltdel_value.to_s == 'true'
              ...
              it { should contain_file('inittab').without_content(%r{^\s*ca}) }
            end
