...
describe 'inittab' do
  describe 'with parameter default_runlevel' do
    ['0', '1', '2', '3', '4', '5', '6', 's', 'S'].each do |runlevel|
      context "set to #{runlevel}" do
        it "is_expected.to not fail on runlevel #{runlevel}" do
          expect { runlevel }.not_to raise_error
        end
      end
    end
  end
  ...
  describe 'with default_runlevel set to invalid value' do
    ...
    it 'is expected to fail' do
      expect {
        is_expected.to contain_class('inittab')
      }.to raise_error
    end
  end
  ...
  describe 'with file_mode specified' do
    context 'as a valid mode' do
      ...
      it {
        is_expected.to contain_file('inittab').with
        {
          ensure:  'file',
          path:    '/etc/inittab',
          owner:   'root',
  ...
  describe 'with parameter require_single_user_mode_password' do
    [true, false].each do |value|
      context "set to #{value}" do
        ...
        if value.to_s == 'true'
          it { is_expected.to contain_file('inittab').with_content(%r{~~:S:wait:/sbin/sulogin}) }
        end
        if value.to_s == 'false'
          it { is_expected.to contain_file('inittab').without_content(%r{~~:S:wait:/sbin/sulogin}) }
        end
      end
    end
  ...
  describe 'with ctrlaltdel_override_path specified' do
    context 'as a valid path' do
      ...
      it {
        is_expected.to contain_file('ctrlaltdel_override').with
        {
          ensure: 'file',
  ...
  describe 'with default values for parameters on Ubuntu' do
    ...
    it { is_expected.to contain_class('inittab') }

    it { is_expected.not_to contain_file('inittab') }

    it {
      is_expected.to contain_file('rc-sysinit.override').with
      {
        ensure:  'file',
    ...
    it { is_expected.to contain_file('rc-sysinit.override').with_content(inittab_fixture) }

    it { is_expected.not_to contain_file('ctrlaltdel_override') }
  end
      ...
      context "except for enable_ctrlaltdel which is set to #{enable_ctrlaltdel_value}" do
        platforms.sort.each do |k, v|
          ...
          context "#{v[:osfamily]} #{v[:release]}" do
            ...
            it { is_expected.not_to contain_file('rc-sysinit.override') }
            ...
            if v[:support_ctrlaltdel_override].to_s == 'true' && enable_ctrlaltdel_value.to_s == 'false'
              it {
                is_expected.to contain_file('ctrlaltdel_override').with
                {
                  ensure: 'file',
              ...
              it { is_expected.to contain_file('inittab').with_content(inittab_fixture) }

              it { is_expected.to contain_file('ctrlaltdel_override').with_content(ctrlaltdel_override_fixture) }
            end

            if v[:support_ctrlaltdel_override].to_s == 'true' && enable_ctrlaltdel_value.to_s == 'true'
              it {
                is_expected.to contain_file('ctrlaltdel_override').with
                {
                  ensure: 'absent',
              ...
              it { is_expected.to contain_file('inittab').without_content(%r{/^\s*ca/}) }
            end
            ...
            if v[:systemd] == true
              ...
              it do
                is_expected.to contain_file('/etc/systemd/system/default.target').with
                {
                  ensure: 'link',
              ...
              it do
                is_expected.to contain_file('/etc/systemd/system/ctrl-alt-del.target').with
                {
                  ensure: 'link',
                  target: '/dev/null',
                }
              ...
              it { is_expected.not_to contain_file('/etc/systemd/system/ctrl-alt-del.target') }
            end
          end
        end
      end
  ...
  describe 'ttys1 service and file for EL6' do
    context 'with ensure_ttys1 set to present' do
      ...
      it {
        is_expected.to contain_file('ttys1_conf').with
        {
          ensure: 'present',
      ...
      it {
        is_expected.to contain_service('ttyS1').with
        {
          ensure:     'running',
          hasstatus:  'false',
          hasrestart: 'false',
    ...
    context 'with ensure_ttys1 set to absent' do
      ...
      it { is_expected.not_to contain_service('ttyS1') }
    end
    ...
    context 'with ensure_ttys1 undefined' do
      ...
      it { is_expected.not_to contain_file('ttys1_conf') }
